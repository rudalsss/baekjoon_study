...
public class Main {
    public static void main(String[] args) {
        ...
        try {
            ...
            Integer[] index_values = new Integer[n]; //좌표순서
            ...
            while(a>0){
                for(int i=0; i<a-1; i++){
                    if(x_values[i] > x_values[i+1]){
                        ...
                        y_values[i] = temp;
                    } else if(x_values[i] == x_values[i+1]){
                        if(y_values[i] >y_values[i+1]){
                            int temp = y_values[i+1];
                            y_values[i+1] = y_values[i];
                            y_values[i] = temp;
            ...
            Arrays.sort( index_values, (a,b) -> {
                ...
                if ( x_values[a] !=  x_values[b] ) return Integer.compare(x_values[a], x_values[b]);
                // x값이 같다면 y값으로 오름차순 정렬
                return Integer.compare(y_values[a], y_values[b]);
            });
